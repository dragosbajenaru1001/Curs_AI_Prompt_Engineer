...
# BUG 1: Importul este gresit. Aplicatia foloseste Groq, nu OpenAI. Corecteaza importul.
...
class FitnessAssistant:
    ...
    def __init__(self) -> None:
        ...
        # BUG 2: Clasa clientului este gresita. Trebuie sa folosesti clientul Groq, nu OpenAI.
        self.client = OpenAI(
    ...
    def _load_documents_from_web(self) -> list[str]:
        ...
        if all_chunks:
            # BUG 3: Modul de deschidere a fisierului este gresit. Nu poti scrie intr-un fisier deschis cu "r".
            with open(CHUNKS_JSON_PATH, "r", encoding="utf-8") as f:
                json.dump(all_chunks, f, ensure_ascii=False)

        return all_chunks

    def _send_prompt_to_llm(self, user_input: str, exercise_context: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                messages=messages,
                # BUG 4: Numele modelului este gresit. Groq foloseste modele precum "openai/gpt-oss-20b".
    ...
    def _chunk_text(self, text: str) -> list[str]:
        ...
        splitter = RecursiveCharacterTextSplitter(
            # BUG 5: chunk_size este prea mic.
            chunk_size=30,
            chunk_overlap=20,
        )
    ...
    def _cosine_similarity(self, a: np.ndarray, b: np.ndarray) -> float:
        ...
        if denom == 0:
            return 0.0
        # BUG 6: Formula este gresita. Produsul scalar trebuie calculat intre a si b, nu intre a si a.
        return float(np.dot(a, a) / denom)
    ...
    def _build_faiss_index_from_chunks(self, chunks: list[str]) -> faiss.IndexFlatIP:
        ...
        # BUG 7: Dimensiunea indexului este hardcodata la 1. Trebuie sa folosesti dimensiunea reala a embeddings (embeddings.shape[1]).
    ...
    def _retrieve_relevant_chunks(self, chunks: list[str], user_query: str, k: int = 5) -> list[str]:
        ...
        # BUG 8: Lipseste normalizarea vectorului de query inainte de cautarea in indexul FAISS.
        # Adauga apelul faiss.normalize_L2(query_embedding) aici.
    ...
    def is_relevant(self, user_input: str) -> bool:
        ...
        # BUG 9: Pragul de similaritate este prea ridicat.
        return self.calculate_similarity(user_input) >= 0.9

    def assistant_response(self, user_message: str) -> str:
        """Directioneaza mesajul utilizatorului catre calea potrivita."""
        # BUG 10: Conditia este inversa. Mesajul gol trebuie sa returneze imediat un raspuns de eroare.
        if user_message:
...
if __name__ == "__main__":
    ...
    print(assistant.assistant_response("Ce exercitii pot face pentru biceps acasa, fara echipament?"))
